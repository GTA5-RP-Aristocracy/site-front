...
const checkCurrentPassword = async () => {
  try {
    const response = await fetch('http://185.218.0.121:8080/user/check_current_password', {
      method: 'POST',
      headers: {
...
const changePassword = async () => {
  try {
    const response = await fetch('http://185.218.0.121:8080/user/change_password', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
function validationPasswordStrength() {
  ...
  if(invalidChar) {
    errorMessageNewPassword.value = 'Cyrillic and inappropriate characters are not allowed';
...
async function MainValidation() {
  ...
  if (newPassword.value.length < 6) {
    errorMessageNewPassword.value = 'Password must be at least 6 characters long';
  } 
  // Проверяем, соответствует ли пароль регулярному выражению (без кириллицы и со спецсимволами)
    ...
    errorMessageNewPassword.value = 'Password must contain at least one special character and no Cyrillic letters';
  }
...
<template>
  <div class="SettingsPopUp">
    <div class="SettingsPopUp__container">
      ...
      <input type="password" placeholder="New password" v-model="newPassword" @focus="clearNewPasswordError" @input="validationPasswordStrength"> 
      <p v-if="errorMessageNewPassword" class="error">{{ errorMessageNewPassword }}</p>

      <!-- Шкала сложности пароля -->
      <div class="SettingsPopUp__password-strength-bar">
        <div :class="passwordStrengthClass" class="SettingsPopUp__password-strength-fill"></div>
      </div>
      <p class="SettingsPopUp__security-level" v-if="newPassword.length > 0 && !errorMessageNewPassword">{{ passwordStrengthText }}</p>
      
      <!-- Подтверждение нового пароля -->
      <input type="password" placeholder="New password confirmation" v-model="testPassword" @focus="clearTestPasswordError" @input="CheckTestPassword">
      <p v-if="errorMessageTestPassword" class="error">{{ errorMessageTestPassword }}</p>
      
      <!-- Кнопка сохранения изменений -->
      <button class="SettingsPopUp__btn" @click.prevent="MainValidation">Save changes</button>
    </div>
  </div>
</template>
