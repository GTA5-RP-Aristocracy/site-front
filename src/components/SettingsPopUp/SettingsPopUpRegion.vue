...
onMounted(async () => {
	try {
		...
		countries.value = data.map((country: Country) => country.name.common)
	} catch (error) {
	...
	if (selectedCountry.value) {
		try {
			const response = await fetch(
				'https://your-server-endpoint.com/update-country',
				{
					method: 'POST',
					headers: {
...
<template>
	<div class="SettingsPopUpRegion" @click.self="closePopUp">
		<div class="SettingsPopUpRegion__container">
			<div
				class="SettingsPopUpRegion__country-select"
				@click="isOpen = !isOpen"
			>
				<span>{{
			...
			<button class="SettingsPopUpRegion__btn" @click="submitChanges">
				Request changes
			</button>
		</div>
	</div>
